...
elseif ($Rainbow) {
    ...
    $heheText = @(
        ...
        '                                                ',
        ' ___ _  _ _ ___ ____ ___  _ _    ____ ___       ',
        ' |__| |  | |  |  |  | |__] | |    |  |  |       ',
        ' |  | |__| |  |  |__| |    | |___ |__|  |       ',
        '                                                ',
        ' ____ _  _ ___  ____ ____ _ ____ _  _ ____ ____ ',
    ...
    Write-Host "`n"
    for ($i = 0; $i -lt 15; $i++) {
        $line = $palm[$i] + $heheText[$i] + $palm[$i]
        $cIndex = 0
        foreach ($char in $line.ToCharArray()) {
            Write-Host $char -ForegroundColor $rainbowColors[$cIndex % $rainbowColors.Count] -NoNewline
            if ($char -match '\S') { $cIndex++ }
        }
        Write-Host ""
    }
    Write-Host "`n"
